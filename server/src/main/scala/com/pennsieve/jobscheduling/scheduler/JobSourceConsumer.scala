...
class JobSourceConsumer(
  ...
  private def safeListTasksAsync: Future[ListTasksResult] =
    ports
      .listTasks(listTasksRequest)
      .flatMap {
        case Right(listTask) =>
          log.tierNoContext.info(
            s"Successfully listed tasks for: ${listTask.getTaskArns.size} tasks"
          )
        ...
        case Left(left) =>
          ...
          left.tailRecM[Future, ListTasksResult] { _ =>
  ...
  private def recurseTilCapacityAvailable: Future[Unit] =
    safeListTasksAsync.flatMap { initialListTasksResult =>
      Monad[Future].tailRecM(initialListTasksResult) { listTasksResult =>
        if (listTasksResult.getTaskArns.size < maxTasks) {
          log.tierNoContext.info(
